...
async def start_render(req: RenderRequest, background_tasks: BackgroundTasks):
    ...
    normalized_audio_path = _normalize_local_path(req.audio_path)
    audio_p = Path(normalized_audio_path)
    if not audio_p.exists():
        raise HTTPException(status_code=422, detail={"error": "file_not_found", "message": f"Audio file not found: {normalized_audio_path}"})
    ...
    background_tasks.add_task(render_visualizer, job_id, normalized_audio_path, cfg)
    return RenderResponse(job_id=job_id)


@router.get("/preview-source")
async def preview_source(audio_path: str = Query(min_length=1)):
    """Stream selected local audio for frontend preview analyser."""
    normalized_audio_path = _normalize_local_path(audio_path)
    audio_p = Path(normalized_audio_path)
